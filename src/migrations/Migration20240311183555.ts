...
export class Migration20240311183555 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      "create table `trainer` (`uuid` text not null, `id` integer not null, `created_at` datetime not null, `updated_at` datetime not null, `name` text not null, `nickname` text null, primary key (`uuid`, `id`));",
    );
    ...
    this.addSql(
      "create table `trainer_pokedex` (`trainer_uuid` text not null, `trainer_id` integer not null, `pokemon_uuid` text not null, constraint `trainer_pokedex_trainer_uuid_trainer_id_foreign` foreign key(`trainer_uuid`, `trainer_id`) references `trainer`(`uuid`, `id`) on delete cascade on update cascade, constraint `trainer_pokedex_pokemon_uuid_foreign` foreign key(`pokemon_uuid`) references `pokemon`(`uuid`) on delete cascade on update cascade, primary key (`trainer_uuid`, `trainer_id`, `pokemon_uuid`));",
    );
    this.addSql(
      "create index `trainer_pokedex_pokemon_uuid_index` on `trainer_pokedex` (`pokemon_uuid`);",
    );
    this.addSql(
      "create index `trainer_pokedex_trainer_uuid_trainer_id_index` on `trainer_pokedex` (`trainer_uuid`, `trainer_id`);",
    );
  }
}
